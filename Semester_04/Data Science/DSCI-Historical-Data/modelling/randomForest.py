...
file_path = os.path.join(current_directory, 'rfc.pickle')  # Change the file path for the Random Forest model
file_path2 = os.path.join(current_directory, 'rfc_output.pickle')  # Change the file path for the output
...
if submit:
    ...
    random_forest_model = RandomForestClassifier(n_estimators=100, criterion='entropy', max_depth=12, min_samples_leaf=1, min_samples_split=3)
    ...
    with open(file_path, 'wb') as rf_pickle:
        pickle.dump(random_forest_model, rf_pickle)
        rf_pickle.close()
    ...
    fig, ax = plt.subplots()
    ax = sns.barplot(x=random_forest_model.feature_importances_, y=features.columns)
    ...
    fig.savefig('rfc_feature_importance.png')

    st.image('rfc_feature_importance.png')
